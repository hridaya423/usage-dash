    ...
    <AnimatePresence>
      {show && items.length > 0 && (
        <motion.div
          ...
          className={cn(
            "pointer-events-none absolute z-10 w-max max-w-[280px] rounded-md border px-2 py-1 shadow-sm",
            VARIANT[variant]
          )}
        >
          {heading && (
          ...
          <div className="flex flex-col gap-0.5">
            {items.map((item) => (
              <div
                key={item.name}
                className="flex items-center gap-1.5 whitespace-nowrap font-mono text-[11px] text-popover-foreground tabular-nums"
